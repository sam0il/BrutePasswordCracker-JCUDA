...
extern "C" __global__ void bruteForceKernel(
    int passwordLength,
    const char* charset,
    int charsetLength,
    ...
    unsigned long long globalIdx = startIndex + idx;
    if (globalIdx >= totalCandidates || *foundFlag == 1) return;
    ...
    // FIX: Use global index for candidate generation
    unsigned long long temp = globalIdx;  // Changed from idx to globalIdx
    for (int i = passwordLength - 1; i >= 0; i--) {
        if (maskArray[i] != -1) {
            candidate[i] = maskArray[i];
        } else {
            int ci = temp % charsetLength;
            candidate[i] = charset[ci];
            temp /= charsetLength;
        }
    }
/////////////////////////////////////////////////////////
    ...
    CUDA_MD5_CTX ctx;
    cuda_md5_init(&ctx);
    cuda_md5_update(&ctx, (const unsigned char*)candidate, passwordLength);
    ...
    if (match) {
            // Atomically set the found flag
            int oldVal = atomicCAS(foundFlag, 0, 1);
            if (oldVal == 0) {  // Only first successful thread proceeds
                for (int i = 0; i < passwordLength; i++) {
                    foundPassword[i] = candidate[i];
                }
                foundPassword[passwordLength] = '\0';
            }
        }
    }
/////////////////////////////////////////////////////////
